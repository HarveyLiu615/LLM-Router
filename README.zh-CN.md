...
DesensitizeProxy 是一个基于 ASP.NET Core 和 YARP 的隐私保护反向代理。你的客户端仍然可以使用 OpenAI-compatible、Anthropic、Gemini 或 Vertex 风格接口；代理会在请求转发到上游 LLM 前，先改写请求体中的文本内容，并按目标提供商自动补齐鉴权头。
...
- OpenAI-compatible 路径，例如 `/v1/chat/completions` 和 `/v1/responses`，匹配 `openai` 或 `openai-compatible` target。
